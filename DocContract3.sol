...
contract DocContract2 {
    ...
    function setForm(string Text) returns (uint256)
    {
        ++blockID;
        docBlock[blockID] = DocumentBlock(Text);
    
        senderForms[msg.sender].push(blockID);
        return blockID;
    }
    function getForm(uint256 index) constant returns (string  strText)
    {
        strText  = "";
        if (index >= senderForms[msg.sender].length) return;
        
        DocumentBlock block = docBlock[senderForms[msg.sender][index]];
    
        strText  = block.blockText;
    }
    function getFormsCount() constant returns (uint256)
    {
        return senderForms[msg.sender].length;
    }
    ...
    mapping(address => uint256[]) senderForms;
